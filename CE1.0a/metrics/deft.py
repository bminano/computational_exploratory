'''
Density Estimation using Field Theory (DEFT) in 1D and 2D (v1.0)
Wrapped for the Computational Exploratory
...
def deft(data, g, minLimit, maxLimit, alpha=2):
    '''
    Calculates a PDF based on the DEFT algorithm.
    ...
    minLimit        Minimal possible value
    maxLimit        Maximal possible value
    ...
    result = np.zeros((data.shape[0], g))

    bbox = [minLimit, maxLimit]
    xgrid = sp.linspace(bbox[0], bbox[1], g)

    for i in xrange(data.shape[0]):
        if len(data[i]) > 1:
            da = deft_1d(data[i], bbox, G=g, alpha=alpha)(xgrid)
            result[i, :] = da
